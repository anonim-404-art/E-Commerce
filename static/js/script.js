...
// Color Tag Input Logic
document.addEventListener("DOMContentLoaded", function () {
  const colorInput = document.getElementById("color-input");
  const container = document.getElementById("color-tag-container");
  const hiddenInput = document.getElementById("colors-hidden-input");
  let colors = [];
  ...
  function createTag(colorName) {
    const tag = document.createElement("div");
    tag.className =
      "flex items-center bg-indigo-50 text-indigo-700 px-4 py-1.5 rounded-full text-xs font-bold border border-indigo-100 animate-in fade-in zoom-in duration-200";
    tag.innerHTML = `
            <span>${colorName}</span>
            <button type="button" class="ml-2 hover:text-red-500 transition-colors focus:outline-none">&times;</button>
        `;
