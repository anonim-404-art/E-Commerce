...
if database_url:
    ...
    app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:123@127.0.0.1:5432/main'
...
# Association Table for Many-to-Many relationship between User and Product
db.Table('store',
...
def checkout():
    ...
    display_items = []
    subtotal = 0
    # for displaying session cart items
...
def product():
    ...
    for product in products:
        count = 0
        for order in orders:
            if order.product_info == product.id:
                count += 1
    ...
    if request.method == "POST":
        ...
        colors_json = [selected_color] if selected_color else []
...
def app_data():
    ...
    if not data:
        return jsonify({"available": False, "message": "No data received"}), 400
